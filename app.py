...
def tela_login():
    # Interface de login inspirada no HomologaSisTele/TRE-BA, adaptada para o SIGA-COR.
    st.markdown(
        """
        <style>
            .stApp {
            ...
            .siga-login-right {
                ...
                width: 100%;
                background-image:
                    linear-gradient(rgba(0, 66, 103, .34), rgba(0, 66, 103, .34)),
                    url('__BG_B64__');
            ...
            .siga-login-right::before {
                content: "";
                position: absolute;
                inset: 0;
                background: linear-gradient(90deg, rgba(0,77,111,.18), rgba(0,77,111,.56));
            }
            .siga-login-brand-corner {
                position: absolute;
                right: 30px;
                bottom: 24px;
                display: flex;
            ...
            .siga-login-flag {
                ...
                border-radius: 4px;
                background:
                    radial-gradient(circle at 52% 50%, #1149ca 0 33%, transparent 34%),
                    linear-gradient(135deg, #0a8f49 0 50%, #ffd51d 51% 100%);
                box-shadow: 0 2px 8px rgba(0,0,0,.28);
            }
            div[data-testid="stVerticalBlock"]:has(.siga-login-left-shell) {
                background: #ffffff;
            }
            div[data-testid="column"]:has(.siga-login-right) {
                padding: 0 !important;
            }
            .siga-login-left-shell [data-testid="stTabs"] button {
                font-size: .84rem;
            }
            .siga-login-left-shell div[data-testid="stTextInput"] input {
                border-color: rgba(0, 94, 122, .42) !important;
            }
            .siga-login-left-shell .stButton > button {
                width: 100%;
            ...
            .siga-login-left-shell .stButton > button:hover {
        ...
        """.replace("__BG_B64__", f"data:image/png;base64,{FUNDO_LOGIN_BASE64}"),
        unsafe_allow_html=True
    )
    ...
    col_login, col_imagem = st.columns([0.31, 0.69], gap="large")

    with col_login:
        st.markdown(
            f"""
            <div class="siga-login-left-shell">
                <div class="siga-login-logo-area">
                    <img src="data:image/png;base64,{LOGO_CORREGEDORIA_BASE64}" alt="Corregedoria Regional Eleitoral da Bahia">
                    <h1 class="siga-login-title">SIGA-COR</h1>
                    <div class="siga-login-subtitle">Sistema Integrado de Gestão de Atendimentos da Corregedoria</div>
                    <div class="siga-login-version">Versão 1.0</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True
        ...
        with aba_login:
            email = normalizar_email(st.text_input("Usuário", placeholder="E-mail institucional", key="login_email_visual"))
            senha = st.text_input("Senha", type="password", key="login_senha_visual")

            if st.button("Login", type="primary", key="btn_login_visual"):
                lista = usuarios()
                encontrado = None
            ...
            st.markdown(
                """
                <div class="siga-login-panel-note">
                    Atenção: acesso restrito a usuários autorizados da Justiça Eleitoral.
                </div>
                """,
                unsafe_allow_html=True
            )
        ...
        with aba_cadastro_zona:
            ...
            with st.form("form_cadastro_externo_zona"):
                ...
                with col1:
                    zona = st.selectbox("Zona eleitoral", opcoes_zonas_nacionais(TRIBUNAL_PADRAO), key="cad_ext_zona")
                    n_zona = numero_zona_eleitoral(zona)
                    nome_padrao = f"{n_zona:03d}ª Zona Eleitoral" if n_zona else ""
                    nome = st.text_input("Nome de exibição", value=nome_padrao, key="cad_ext_nome")
                with col2:
                    email_padrao = email_zona_eleitoral(zona) if zona and zona != "Não informado" else ""
                    email = normalizar_email(st.text_input("E-mail institucional da zona", value=email_padrao, key="cad_ext_email"))
                ...
                st.caption("Sugestão: use o e-mail institucional da zona no padrão zonaXXX@tre-ba.jus.br. O vínculo da zona será usado para restringir a visualização dos atendimentos.")
                ...
                if not zona or zona == "Não informado" or not numero_zona_eleitoral(zona):
                    ...
                    ok, msg = enviar_email(
                        ...
                        (
                            ...
                            f"{link}\n\n"
                            "Este acesso permitirá consultar a base de conhecimento institucional e acompanhar apenas os atendimentos vinculados à própria zona.\n\n"
    ...
    with col_imagem:
        st.markdown(
            f"""
            <div class="siga-login-right" role="img" aria-label="Imagem institucional do Tribunal Regional Eleitoral da Bahia com filtro azul da Corregedoria">
                <div class="siga-login-brand-corner">
                    <img src="data:image/png;base64,{LOGO_CORREGEDORIA_BASE64}" alt="Corregedoria Regional Eleitoral da Bahia">
                    <div class="siga-login-treba"><span class="siga-login-flag"></span><span>TRE-BA</span></div>
                </div>
            </div>
            """,
            unsafe_allow_html=True
...
def css_pacote_visual_1():
    ...
    st.markdown(
        ...
        /* Pacote Visual 2 - Moderno */
        :root {
            --modern-bg-a: #E9F0F8;
            --modern-bg-b: #DDE8F4;
            --modern-panel: rgba(248, 251, 255, .88);
            --modern-panel-solid: #F7FAFE;
            --modern-border: #BFD0E4;
            --modern-border-soft: #D5E2F1;
            --modern-text: #0A2342;
            --modern-muted: #5F7187;
            --modern-blue: #0B5CAD;
            --modern-blue-dark: #063D78;
            --modern-cyan: #12A8E8;
            --modern-shadow: 0 14px 34px rgba(6, 31, 61, .10);
            --modern-shadow-soft: 0 8px 20px rgba(6, 31, 61, .075);
        }

        html, body, [data-testid="stAppViewContainer"] {
            background:
                radial-gradient(circle at 18% 3%, rgba(18,168,232,.16), transparent 22rem),
                radial-gradient(circle at 84% 18%, rgba(11,92,173,.12), transparent 24rem),
                linear-gradient(180deg, var(--modern-bg-a) 0%, var(--modern-bg-b) 100%) !important;
            color: var(--modern-text) !important;
        }

        [data-testid="stHeader"] {
            background: rgba(233, 240, 248, .82) !important;
            backdrop-filter: blur(18px);
            border-bottom: 1px solid rgba(191, 208, 228, .75) !important;
        }
        ...
        section[data-testid="stSidebar"] {
            background:
                radial-gradient(circle at 90% 6%, rgba(18,168,232,.22), transparent 9rem),
                linear-gradient(180deg, #04182F 0%, #082D53 50%, #06345F 100%) !important;
            border-right: 1px solid rgba(255,255,255,.10) !important;
            box-shadow: 18px 0 40px rgba(4,24,47,.20) !important;
        }
        ...
        .siga-sidebar-current {
            ...
            border-radius: 18px !important;
            box-shadow: inset 0 1px 0 rgba(255,255,255,.10), 0 10px 22px rgba(0,0,0,.08) !important;
        }
        ...
        section[data-testid="stSidebar"] .stButton > button:hover {
            background: rgba(18,168,232,.22) !important;
            border-color: rgba(95,197,241,.42) !important;
        ...
        .siga-home-hero {
            background:
                radial-gradient(circle at 84% 0%, rgba(255,255,255,.22), transparent 14rem),
                linear-gradient(135deg, #04182F 0%, #0B4D88 48%, #0F94D6 100%) !important;
            border: 1px solid rgba(255,255,255,.24) !important;
            box-shadow: 0 18px 40px rgba(6,31,61,.16) !important;
        ...
        .siga-card-nav:hover {
            ...
            border-color: rgba(11,92,173,.42) !important;
            box-shadow: var(--modern-shadow) !important;
        }

        .siga-card-tag {
            background: #E4F1FF !important;
            border: 1px solid #BFDDFB !important;
        ...
        [data-baseweb="select"] > div:focus-within {
            border-color: rgba(11,92,173,.64) !important;
            box-shadow: 0 0 0 3px rgba(11,92,173,.12) !important;
        }
        ...
        .atendimento-card-header {
            background: linear-gradient(90deg, #E2ECF8, rgba(247,250,254,.90)) !important;
        ...
        .stButton > button[kind="secondaryFormSubmit"],
        button[data-testid="baseButton-primary"] {
            background: linear-gradient(135deg, var(--modern-blue-dark), var(--modern-blue)) !important;
            border: 1px solid rgba(255,255,255,.10) !important;
            border-radius: 12px !important;
            min-height: 40px !important;
            padding: 0 18px !important;
            font-weight: 850 !important;
            box-shadow: 0 10px 20px rgba(11,92,173,.20) !important;
        }
        ...
        .stButton > button[kind="secondaryFormSubmit"]:hover,
        button[data-testid="baseButton-primary"]:hover {
            background: linear-gradient(135deg, #052F61, #084F98) !important;
            box-shadow: 0 12px 24px rgba(11,92,173,.26) !important;
        ...
        .stButton > button:not([kind="primary"]):not([kind="primaryFormSubmit"]):not([kind="secondaryFormSubmit"]) {
            ...
            -webkit-text-fill-color: #073B68 !important;
            border: 1px solid var(--modern-border) !important;
            border-radius: 12px !important;
            box-shadow: none !important;
        }
        ...
        /* Pacote Visual 3 - Painel Executivo */
        :root {
            --exec-bg: #EEF3F8;
            --exec-bg-soft: #E6EDF5;
            --exec-surface: #FFFFFF;
            --exec-surface-2: #F8FBFE;
            --exec-border: #D8E1EC;
            --exec-border-strong: #C4D1E0;
            --exec-text: #102A43;
            --exec-muted: #5B7088;
            --exec-primary: #114B8B;
            --exec-primary-dark: #0B3461;
            --exec-accent: #1F78D1;
            --exec-success: #0F766E;
            --exec-warning: #B45309;
            --exec-shadow: 0 10px 30px rgba(16, 42, 67, 0.08);
            --exec-shadow-soft: 0 6px 18px rgba(16, 42, 67, 0.06);
        }

        html, body, [data-testid="stAppViewContainer"] {
            background:
                linear-gradient(180deg, var(--exec-bg) 0%, var(--exec-bg-soft) 100%) !important;
        ...
        section[data-testid="stSidebar"] {
            background:
                linear-gradient(180deg, #0A1F36 0%, #0E3359 54%, #123F70 100%) !important;
            border-right: 1px solid rgba(255,255,255,.08) !important;
            box-shadow: 18px 0 40px rgba(10, 31, 54, .18) !important;
        }
        ...
        section[data-testid="stSidebar"] .stButton > button:hover {
            background: rgba(31,120,209,.20) !important;
            border-color: rgba(112,177,238,.42) !important;
        ...
        .siga-home-hero {
            background: linear-gradient(135deg, #0C2D4D 0%, #114B8B 60%, #1766B8 100%) !important;
            border: 1px solid rgba(255,255,255,.14) !important;
            box-shadow: 0 14px 36px rgba(17, 75, 139, .18) !important;
            border-radius: 24px !important;
        }
        ...
        .siga-card-nav::before {
            ...
            background: linear-gradient(90deg, var(--exec-primary-dark), var(--exec-accent)) !important;
        }

        .siga-card-nav:hover {
            transform: translateY(-2px) !important;
            border-color: var(--exec-border-strong) !important;
        ...
        .siga-card-tag {
            ...
            color: var(--exec-primary-dark) !important;
        ...
        [data-testid="stTextArea"] textarea:focus,
        [data-baseweb="select"] > div:focus-within {
            border-color: rgba(17,75,139,.56) !important;
            box-shadow: 0 0 0 3px rgba(17,75,139,.10) !important;
        }
        ...
        div[data-testid="stMetric"]::before {
            ...
            width: 100% !important;
            height: 4px !important;
            background: linear-gradient(90deg, var(--exec-primary), var(--exec-accent)) !important;
        }
        ...
        .atendimento-card-header {
            background: linear-gradient(90deg, #F2F7FC, #FFFFFF) !important;
            border-bottom: 1px solid var(--exec-border) !important;
        }
        ...
        .stButton > button[kind="secondaryFormSubmit"],
        button[data-testid="baseButton-primary"] {
            background: linear-gradient(135deg, var(--exec-primary-dark), var(--exec-primary)) !important;
            border: 1px solid rgba(255,255,255,.08) !important;
            border-radius: 12px !important;
            min-height: 40px !important;
            padding: 0 18px !important;
            font-weight: 800 !important;
            box-shadow: 0 8px 18px rgba(17,75,139,.16) !important;
        }
        ...
        .stButton > button[kind="secondaryFormSubmit"]:hover,
        button[data-testid="baseButton-primary"]:hover {
            background: linear-gradient(135deg, #092B50, #0D437C) !important;
            box-shadow: 0 10px 20px rgba(17,75,139,.22) !important;
            transform: translateY(-1px) !important;
        }
        ...
        .stButton > button:not([kind="primary"]):not([kind="primaryFormSubmit"]):not([kind="secondaryFormSubmit"]) {
            background: #FFFFFF !important;
            color: var(--exec-primary-dark) !important;
            -webkit-text-fill-color: var(--exec-primary-dark) !important;
            border: 1px solid var(--exec-border) !important;
            border-radius: 12px !important;
        }
...
def normalizar_secao_livre(valor):
    """
    Normaliza sigla de seção sem substituir valor desconhecido por SEPRO.
    Usada para comparar dados vindos do banco.
    """
    texto = str(valor or "").strip().upper()
    if not texto:
        return ""
    ...
    # Remove descrições longas que possam ter sido gravadas junto com a sigla.
    for sigla in ["SEPRO", "SEOCE", "SEORZE", "COSCAD", "COAJUC", "COORZE", "SCR", "CGE", "CRE"]:
        if texto == sigla or texto.startswith(sigla + " ") or texto.startswith(sigla + " -") or texto.startswith(sigla + "–"):
            if sigla == "SEORZE":
                return "SEOCE"
            return sigla
...
def supabase_get_admin_silencioso(tabela, params=None):
    """
    Leitura silenciosa usando service role quando configurada.
    Ajuda quando RLS impede leitura pelo anon key.
    """
    try:
        headers = supabase_write_headers() if "supabase_write_headers" in globals() else supabase_headers()
        resp = requests.get(
